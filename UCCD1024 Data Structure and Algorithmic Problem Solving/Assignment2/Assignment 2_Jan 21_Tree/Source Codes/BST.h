...
struct BST {
		...
		void balancing(BTNode *);			// sort deepest unbalance tree
		
		// New add
		bool deepestNodes();			// print deepest node in BST

		bool display(int, int);			// print the content of a BST
		bool ascendingPrint(ostream &);	// print node by ascending order
		void ascendingPrint2(BTNode *, ostream &);	// recursive function for ascendingPrint(ostream &)
		bool descendingPrint(ostream &);			// print node by descending order
		void descendingPrint2(BTNode *, ostream &);	// recursive function for descendingPrint(ostream &)

		bool CloneSubtree(BST, type);     // clone the t1 to t2
		void CloneSubtree2(BTNode *, type);    // recursive function for CloneSubtree2()
		void CloneSubtree3(BTNode *);         // recursive function for CloneSubtree3()

		bool printAncestor(type);            // print the ancestors for given node
		bool printAncestor2(BTNode *, type); // recursive function for printAncestor(type)

		bool printSpiral();                 // print the node in spiral order
		void printSpiral2(BTNode*, int, int);  // recursive function for printSpiral()
